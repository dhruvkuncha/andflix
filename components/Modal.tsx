    ...
    async function fetchMovie() {
      ...
      if (data?.videos) {
        const index = data.videos.results.findIndex(
          (element: Element) =>
            element.type === "Clip" || element.type === "Trailer"
        );
        setClip(data?.videos.results[index]?.key);
      }
      if (data?.genres) {
        ...
        <div className="flex flex-col space-x-16 rounded-b-md bg-[#181818] px-10 py-8">
            <div className="space-y-6 text-lg">
                <div className="flex items-center space-x-2 text-sm"> 
                    <p className="font-semibold text-green-400">{Math.ceil(movie?.vote_average * 10)}% Match</p>
                    <p className="font-light">{movie?.release_date || movie?.first_air_date}</p>
                    <div className="flex h-4 items-center justify-center rounded border
                     border-white/40 opacity-40 px-1.5 text-xs">HD</div>
                </div>
            </div>
            <div className="flex flex-col gap-x-10 gap-y-4 font-light md:flex-row mt-10">
                <p className="w-5/6 ">
                    {movie?.overview}
                </p>
